export class JSNAMonitorImpl extends UI.VBox {
    constructor() {
        super(true);
        this._nodes = [];
        this._started = false;
        ...
        this._columns = [
            {id: 'name', title: ls`Name`, visible: true, sortable: true, weight: 60},
            ...
            {id: 'params', title: ls`Params`, visible: true, hideable: true, weight: 60},
            {id: 'result', title: ls`Result`, visible: true, hideable: true, weight: 60},
        ...
        this.contentElement.classList.add('jsna-monitor');

        const topToolbar = new UI.Toolbar('jsna-monitor-toolbar', this.contentElement);
        ...
        const split = new UI.SplitWidget(true, true, 'jsna-monitor-panel-split', 250);
        split.show(this.contentElement);
        this._dataGrid = new DataGrid.SortableDataGrid({displayName: ls`jsNative Monitor`, columns: this._columns});
    ...
    _messageRecieved(message, target) {
        ...
        if ('id' in message) {
            ...
            node.data.result = message.result || message.error;
...
export class InfoWidget extends UI.VBox {
    constructor() {
        ...
        this._tabbedPane.selectTab('result');
        this.render(null);
    }

    render(data) {
        const requestEnabled = data && data.direction === 'sent';
        this._tabbedPane.setTabEnabled('params', !!requestEnabled);
        if (!data) {
