...
export class MainImpl {
    ...
    _createSettings(prefs) {
        ...
        if (Host.isCustomDevtoolsFrontend()) {
            ...
            !Root.Runtime.queryParam('can_dock') &&
            !!Root.Runtime.queryParam('debugFrontend') &&
            !Host.isUnderTest()
        ) {
    ...
    _initializeExperiments() {
        ...
        Root.Runtime.experiments.register(
            ...
            'Background services section for Notifications'
        );
        Root.Runtime.experiments.register(
            'backgroundServicesPaymentHandler',
            'Background services section for Payment Handler'
        );
        Root.Runtime.experiments.register(
            'backgroundServicesPushMessaging',
            'Background services section for Push Messaging'
        );
    ...
    async _createAppUI() {
        ...
        // warn 隐藏头部
        UI.inspectorView._drawerSplitWidget._mainWidget._headerElement.classList.add('hidden');
        ...
        SDK.targetManager.addEventListener(
            SDK.TargetManager.Events.SuspendStateChanged,
    ...
    _showAppUI(appProvider) {
        ...
        if (toggleSearchNodeAction) {
            Host.InspectorFrontendHost.events.addEventListener(
                ...
                toggleSearchNodeAction.execute.bind(toggleSearchNodeAction),
                this
            );
        }
        Host.InspectorFrontendHost.events.addEventListener(
    ...
    _registerForwardedShortcuts() {
        ...
        const actionKeys = UI.shortcutRegistry
    ...
    _registerShortcuts() {
        ...
        const inspectElementModeShortcuts = UI.shortcutRegistry.shortcutDescriptorsForAction(
    ...
    _handleContextMenu(contextMenu) {
        if (Components.dockController.canDock()) {
            ...
            undock.addEventListener(
                ...
                setDockSide.bind(null, Components.DockController.State.Undocked)
            );
            ...
            left.addEventListener(
                UI.ToolbarButton.Events.Click,
                setDockSide.bind(null, Components.DockController.State.DockedToLeft)
            );
        ...
        if (
            ...
            SDK.targetManager.mainTarget().type() === SDK.Target.Type.Frame
        ) {
            contextMenu
                .defaultSection()
                .appendAction('inspector_main.focus-debuggee', Common.UIString('Focus debuggee'));
        }
        ...
        contextMenu
            ...
            .appendAction(
                'main.toggle-drawer',
                UI.inspectorView.drawerVisible()
