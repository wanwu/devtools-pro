...
export class RequestInitiatorView extends UI.VBox {
    ...
    constructor(request) {
        super();
        this.registerRequiredCSS('network-standalone/requestInitiatorView.css');
    ...
    static createStackTracePreview(request, linkifier, focusableLink, callback) {
        ...
        const stackTrace = Components.JSPresentationUtils.buildStackTracePreviewContents(target, linkifier, {
            stackTrace: initiator.stack,
            contentUpdated: callback,
    ...
    wasShown() {
        ...
        if (initiatorGraph.initiators.size > 1 || initiatorGraph.initiated.size > 1) {
            ...
            this._appendExpandableSection(
                this._buildRequestChainTree(initiatorGraph).element,
                ls`Request initiator chain`,
                true
            );
        }
