...
export class NetworkConfigView extends UI.VBox {
    constructor() {
        super(true);
        this.registerRequiredCSS('network-standalone/networkConfigView.css');
    ...
    static createUserAgentSelectAndInput(title) {
        ...
        for (const userAgentDescriptor of userAgentGroups) {
            ...
            for (const userAgentVersion of userAgentDescriptor.values) {
                const userAgentValue = SDK.MultitargetNetworkManager.patchUserAgentWithChromeVersion(
    ...
    _createNetworkThrottlingSection() {
        ...
        const networkThrottlingSelect = /** @type {!HTMLSelectElement} */ (section.createChild(
            'select',
            'chrome-select'
        ));
