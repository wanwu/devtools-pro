...
export class NetworkWaterfallColumn extends UI.VBox {
    ...
    constructor(calculator) {
        ...
        super(false);
        this.registerRequiredCSS('network-standalone/networkWaterfallColumn.css');
    ...
    static _buildResourceTypeStyle() {
        ...
        for (const resourceType of Object.values(Common.resourceTypes)) {
            ...
            waitingStyleMap.set(resourceType, {
    ...
    _getPopoverRequest(event) {
        ...
        let end;
        if (useTimingBars) {
            range = RequestTimingView.calculateRequestTimeRanges(request, 0).find(
                data => data.name === RequestTimeRangeNames.Total
            );
        ...
        const y =
            this._headerHeight + (this._rowHeight * rowIndex - this._scrollTop) + (this._rowHeight - barHeight) / 2;
    ...
    _draw() {
        ...
        const lastRequestIndex = Math.min(
            nodes.length,
            firstRequestIndex + Math.ceil(this._offsetHeight / this._rowHeight)
        );
        ...
        PerfUI.TimelineGrid.drawCanvasHeaders(
            context,
            dividersData,
            time => this._calculator.formatValue(time, dividersData.precision),
            this._fontSize,
            this._headerHeight,
            freeZoneAtLeft
        );
    ...
    _buildSimplifiedBarLayers(context, node, y) {
        ...
        if (!this._calculator.startAtZero) {
            const queueingRange = RequestTimingView.calculateRequestTimeRanges(request, 0).find(
                data => data.name === RequestTimeRangeNames.Total
            );
    ...
    _buildTimingBarLayers(node, y) {
        ...
        for (const range of ranges) {
            if (
                range.name === RequestTimeRangeNames.Total ||
                range.name === RequestTimeRangeNames.Sending ||
