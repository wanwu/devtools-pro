...
export class ResourceWebSocketFrameView extends UI.VBox {
    ...
    constructor(request) {
        super();
        this.registerRequiredCSS('network-standalone/webSocketFrameView.css');
        ...
        this._timeComparator = /** @type {function(!ResourceWebSocketFrameNode, !ResourceWebSocketFrameNode):number} */ (ResourceWebSocketFrameNodeTimeComparator);
    ...
    async _onFrameSelected(event) {
        ...
        this._splitWidget.setSidebarWidget(
            new SourceFrame.ResourceSourceFrame(
...
export class ResourceWebSocketFrameNode extends DataGrid.SortableDataGridNode {
    ...
    createCells(element) {
        element.classList.toggle(
            ...
            this._frame.type === SDK.NetworkRequest.WebSocketFrameType.Error
        );
        element.classList.toggle(
            'websocket-frame-view-row-send',
            this._frame.type === SDK.NetworkRequest.WebSocketFrameType.Send
        );
        element.classList.toggle(
            'websocket-frame-view-row-receive',
            this._frame.type === SDK.NetworkRequest.WebSocketFrameType.Receive
        );
