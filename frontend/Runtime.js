...
class Runtime {
    ...
    static _loadScriptsPromise(scriptNames, base) {
        ...
        for (let i = 0; i < scriptNames.length; ++i) {
            ...
            urls.push(sourceURL);
            const loadResourcePromise = base
                ? Runtime.loadResourcePromiseWithFallback(sourceURL)
                : Runtime.loadResourcePromise(sourceURL);
            promises.push(
    ...
    sharedInstance(constructorFunction) {
        if (
            Runtime._instanceSymbol in constructorFunction &&
